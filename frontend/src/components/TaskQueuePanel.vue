...
async function onTogglePause() {
  const next = !taskStore.paused
  try {
    await taskStore.togglePause(next)
    if (next) {
      simStore.addLog('INFO', '调度器已暂停，新任务将只入队不执行')
    } else {
      simStore.addLog('INFO', '调度器已恢复，队列中的任务将开始执行')
    }
  } catch (err) {
    simStore.addLog('ERROR', '切换暂停状态失败：' + (err.response?.data?.detail || err.message))
  }
}
    ...
    <template v-else>
      <div class="scheduler-config">
        ...
        <div class="field">
          ...
          <button
            class="btn btn-sm"
            :class="taskStore.paused ? 'btn-success' : 'btn-warning'"
            @click="onTogglePause"
          >
            {{ taskStore.paused ? '开始执行队列' : '暂停调度（只入队）' }}
          </button>
        </div>
      </div>
      ...
      <div class="task-stats">
        ...
        <span v-if="taskStore.paused" class="stat-item" style="background:#f0ad4e;color:#fff;">已暂停</span>
      </div>
