...
class SchedulerConfig:
    ...
    paused: bool = False  # 暂停调度：True 时任务只入队不自动启动，等用户手动恢复
...
class TaskScheduler:
    ...
    def _load_config(self) -> None:
        if _SCHEDULER_CONFIG_FILE.exists():
            try:
                ...
                self.config.paused = bool(data.get("paused", False))
    ...
    async def submit(
        ...
        if self.config.mode == "single":
            ...
            if not self.config.paused and len(self._running) < self.config.max_concurrent:
                st.status = "running"
                st.started_at = time.time()
                self._running.add(task_id)
                helios_service.start_task(task_id, st._assets)
