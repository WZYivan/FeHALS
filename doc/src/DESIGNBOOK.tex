...
激光雷达（Light Detection and Ranging，LiDAR）通过发射激光脉冲并测量回波，能够直接获取地表及地物的三维离散采样。根据搭载平台的不同，激光扫描可分为机载（Airborne Laser Scanning，ALS）、车载（Mobile Laser Scanning，MLS）与地基（Terrestrial Laser Scanning，TLS）三类，是数字高程模型生产、森林调查、城市三维建模和灾害监测的重要数据源。实际外业采集受设备成本、空域、天气和任务风险制约，外业前通过虚拟场景试验航线与扫描参数，可以降低反复试飞与试测的成本。
...
FeHALS（Frontend of HELIOS++ for Airborne Laser Scanning，最初面向机载激光扫描，现已扩展支持车载与地基平台）正是在这一背景下提出的，它以浏览器为操作入口，将三维预览、航点编辑、弓字形航线生成、参数配置、HELIOS++任务执行、实时日志和点云分析串联为完整流程，旨在为HELIOS++提供一个现代化、免配置的Web前端套件。开放式WebGIS技术便于跨平台访问与后续扩展\citep{opengis,viswebdrone,lidarWebgis}，浏览器侧利用WebGL和Three.js完成硬件加速三维显示，无需安装专用桌面GIS软件。在GIS应用层面，FeHALS将激光雷达仿真中的坐标系、高程模型、航线规划、点云统计分析等GIS核心概念融入Web工作流，使仿真过程与GIS工作流自然衔接\citep{pointCloudWeb}。
...
系统面向激光雷达仿真学习者、测绘工程学生和方案验证人员，定位为教学与原型验证工具，不替代实际外业作业中的空域审批、飞控、坐标转换及成果质量验收。本设计书以GitHub仓库master分支当前实现为准，规划中的LOD、任务队列、Docker等功能不会当作现成功能描述。
...
一次完整业务流程为：启动前后端并检查HELIOS++环境；上传场景模型并检查方向和包围盒；手工编辑航点或由矩形生成弓字形航线；设置平台、扫描器与扫描参数；生成轨迹和配置；提交仿真并查看实时日志；加载结果点云、查看统计与直方图；下载结果或清理缓存。

在仿真数据源方面，系统可处理两类典型的三维扫描场景数据：一类是室外扫描场景，即建筑群级别的模型集合，如BuildingWorld数据集中的单体建筑模型，这类模型细节丰富、结构复杂，适合用于验证机载与车载激光扫描参数对不同建筑形态的响应差异；另一类是室内场景，即建筑内部结构模型，如Building3D数据集中的精细室内场景，其空间尺度小、遮挡关系复杂，适合用于评估地基激光扫描的激光穿透率和点云完整性。两类场景数据对于验证FeHALS的航高自动计算、扫描角度配置和点云覆盖度分析等功能具有重要参考价值。
...
\begin{table}[pos=!htbp]
    ...
    \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l l}
        ...
        FR-06 & 设置平台（机载/车载/地基）、扫描器型号及速度、高度、扫描频率、扫描角、脉冲频率与输出格式。 \\
...
系统依赖正确安装的HELIOS++及其环境路径，未配置时只能使用三维预览。平台覆盖机载（copter\_linearpath、sr22、quadcopter）、车载（vehicle\_linearpath）与地基静态（tripod、tripod\_down）三类，扫描器覆盖ALS（riegl\_vux-1uav、riegl\_vq\_780i 等）与 TLS/MLS（vlp16、riegl\_vz400、velodyne\_hdl-64e 等）型号，按平台类型生成相应的 survey 结构。航点在$z=0$地面编辑，机载平台生成轨迹时统一叠加飞行高度，属于恒高简化模型；车载平台逐航点贴地（onGround）运动，地基静态平台使用固定坐标扫描。仓库说明记录本机构建的LASlib输出异常，因此默认XYZ；LAS/LAZ只有在HELIOS++正确链接LASlib时可用。任务和配置主要存于单进程内存，系统也未设置账户权限，生产部署前必须补充持久化、鉴权、上传限制、HTTPS和审计。
...
机载平台中，UAV（copter\_linearpath）速度为$0.5$--$50\text{ m/s}$、高度为$3$--$500\text{ m}$；固定翼Airborne（sr22）速度为$20$--$200\text{ m/s}$、高度为$200$--$5000\text{ m}$；车载平台（vehicle\_linearpath）速度为$0.5$--$30\text{ m/s}$、传感器高度为$0.5$--$5\text{ m}$；地基静态平台（tripod）架设高度为$0.5$--$10\text{ m}$。RIEGL VUX-1UAV扫描频率为$10$--$200\text{ Hz}$，扫描半角为$1^\circ$--$165^\circ$，脉冲频率为$50$--$550\text{ kHz}$；Velodyne VLP-16 等 360° 旋转 LiDAR 的扫描频率（即转头转速）为$5$--$20\text{ Hz}$。
...
见\href{https://github.com/WZYivan/FeHALS/blob/master/DESCRIPTION.pdf}{项目描述}的Section~\textcolor{hscolor}{CRediT ~authorship ~contribution ~statement}。
